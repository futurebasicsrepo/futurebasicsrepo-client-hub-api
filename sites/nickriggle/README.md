...
- **Libraries**: GSAP 3.12.5 + ScrollTrigger (cdnjs) drive the hero entrance, word-level heading reveals, staggered section reveals, magnetic buttons, cover tilt, scroll parallax, and the desktop pinned rail. Lenis 1.1.18 (jsdelivr) adds inertial smooth scrolling on pointer devices only. Every effect has a CSS or IntersectionObserver fallback, so the page works fully if the CDNs are unreachable, and everything honours `prefers-reduced-motion`.
